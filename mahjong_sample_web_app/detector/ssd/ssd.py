...
class SingleShotMultiBoxDetector:
    ...
    default_shapes = dict(ssd300=(300, 300, 3))
    ...
    def __init__(
        ...
        if "bg" != class_names[0]:
            print("Warning: Fist label should be bg." " It'll be added automatically.")
    ...
    def build(self, init_weight="keras_imagenet"):
        ...
        if self.model_type == "ssd300" and self.base_net == "vgg16":
            self.model, priors = SSD300_vgg16(
                self.input_shape, self.n_classes, self.aspect_ratios, self.scales
            )
        elif self.model_type == "ssd300" and self.base_net == "resnet50":
            self.model, priors = SSD300_resnet50(
                self.input_shape, self.n_classes, self.aspect_ratios, self.scales
            )
        elif self.model_type == "ssd300" and self.base_net == "xception":
            self.model, priors = SSD300_xception(
                self.input_shape, self.n_classes, self.aspect_ratios, self.scales
            )
        elif self.model_type == "ssd512" and self.base_net == "vgg16":
            self.model, priors = SSD512_vgg16(
                self.input_shape, self.n_classes, self.aspect_ratios, self.scales
            )
        elif self.model_type == "ssd512" and self.base_net == "resnet50":
            self.model, priors = SSD512_resnet50(
                self.input_shape, self.n_classes, self.aspect_ratios, self.scales
            )
        else:
            raise NameError(
        ...
        if init_weight is None:
            ...
            print(
                "Initializing network with keras application model"
                " pretrained imagenet."
            )
            if self.base_net == "vgg16":
                ...
                raise NameError("{} is not defined.".format(self.base_net))
    ...
    def train_by_generator(
        ...
        if checkpoints:
            callbacks.append(
                keras.callbacks.ModelCheckpoint(
                    checkpoints, verbose=1, save_weights_only=True
                )
            )
        ...
        self.model.compile(
            optimizer=optim,
            loss=MultiBoxLoss(self.n_classes, neg_pos_ratio=neg_pos_ratio).compute_loss,
        )
